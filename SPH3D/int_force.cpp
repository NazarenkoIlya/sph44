...
void IntForce::find(int n_total, Particle* particles, int niac, int* pair_i, int* pair_j, double** dwdx, double** dvxdt, double* tdsdt, double* dedt, double* eta)
{
    ...
    if (parameters::visc)
    { 
//#pragma omp parallel for private(hxx, hyy, hxy)  
        for (int k = 0; k < niac; k++)
        {
            ...
            if (parameters::dim == 1)
            {
                hxx = 2.f * dvx[0] * dwdx[0][k];
            }
            else if (parameters::dim == 2)
            {
                hxx = 2.f * dvx[0] * dwdx[0][k] - dvx[1] * dwdx[1][k];
                hxy = dvx[0] * dwdx[1][k] + dvx[1] * dwdx[0][k];
                hyy = 2.f * dvx[1] * dwdx[1][k] - dvx[0] * dwdx[0][k];
            }
...
//#pragma omp parallel for private(pressure)
    for (int i = 0; i < n_total; i++)
    {
        if (parameters::visc)
        {
...
//#pragma omp parallel for private(hxx, hyy, hxy, h, hvcc, he, rhoij)
    for (int k = 0; k < niac; k++)
    {
        int i = pair_i[k];
        int j = pair_j[k];
